 ...
 * @package    Wp_Ssawc
 * @subpackage Wp_Ssawc/admin
 * @author     Bao Dang <dtquocbao@gmail.com>
 */
class Wp_Cbf_Admin {
    ...
    public function add_plugin_admin_menu() {
         ...
         *
         */
        $plugin_screen_hook_suffix = add_options_page( __('SSActiveWear Connector', $this->plugin_name ), 'SSActiveWear', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
        );
    }
    ...
    public function validate($input) {
      ...
      if(isset($input['admin_menu_items'])):
        ...
        $valid['consumer_key'] = (isset($input['consumer_key']) && !empty($input['consumer_key'])) ? wp_kses($input['consumer_key'], array('a' => array( 'href' => array(), 'title' => array()))) : '';

         $valid['consumer_secret'] = (isset($input['consumer_secret']) && !empty($input['consumer_secret'])) ? wp_kses($input['consumer_secret'], array('a' => array( 'href' => array(), 'title' => array()))) : '';
    ...
    *
    ...
    public function wp_cbf_woocommerce_consumer_key($consumer_key){
        if(!empty($this->wp_cbf_options['consumer_key'])){
            $consumer_key = $this->wp_cbf_options['consumer_key'];
        }
        return $footer_text;
    }   

    public function wp_cbf_woocommerce_consumer_secret($consumer_secret){
        if(!empty($this->wp_cbf_options['consumer_secret'])){
            $consumer_secret = $this->wp_cbf_options['consumer_secret'];
        }
        return $footer_text;
    }
